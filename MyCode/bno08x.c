...
//         // 尝试与传感器开始通信
//         if (sendPacket(CHANNEL_EXECUTABLE, 1) == 0) // 发送失败
//         {
//             HAL_Delay(100);
//             attempts++;
//             continue;
...
//         {
//             packetCount++;
//             if (packetCount > 10) break; // 防止无限循环
//         }
        
//         HAL_Delay(200); // 增加延时，给传感器更多初始化时间
...
//         if (reason > 0) // 如果能获取到复位原因，说明复位成功
//         {
//             return; // 复位成功
//         }
        
//         attempts++;
//         HAL_Delay(200); // 尝试之间增加延时
//     }
    
//     // 如果达到最大尝试次数仍然失败，可以在这里添加错误处理代码
// }
...
void WriteNbyte(uint8_t *p, uint8_t number) /*  WordAddress,First Data Address,Byte lenth   */
{
    HAL_I2C_Master_Transmit( &hi2c1, BNO080_W_ADDRESS, p, number, 0xffff );
}

void ReadNbyte(uint8_t *p, uint8_t number) /*  WordAddress,First Data Address,Byte lenth   */
{
    HAL_I2C_Master_Receive( &hi2c1, BNO080_R_ADDRESS, p, number, 0xffff );
}

uint8_t i2c_tmp[32] = {0};
...
char sendPacket(unsigned char channelNumber, unsigned char dataLength)
{
    ...
    WriteNbyte(i2c_tmp, (uint8_t)(dataLength + 4));
